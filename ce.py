...
class CompletionNet(nn.Module):
    def __init__(self, input_nc, norm_layer=nn.BatchNorm2d, gpu_ids=[]):
        super(CompletionNet, self).__init__()
        self.gpu_ids = gpu_ids

        model = [
            # 256x256
            nn.Conv2d(input_nc, 64, kernel_size=5, stride=1, padding=2, bias=False),
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.InstanceNorm2d(256),
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=8, dilation=8, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.InstanceNorm2d(256),
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=16, dilation=16, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.InstanceNorm2d(256),
            # 64x64
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, x):
        if isinstance(x.data, torch.cuda.FloatTensor) and self.gpu_ids:
